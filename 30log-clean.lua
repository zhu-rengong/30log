...
---@class _30log_module
---More detail: https://github.com/Yonaba/30log/wiki/Creating-classes
---@overload fun(name?:string, attr?:{}):_30log_class
local class = {
...
_class = function(name, attr)
    ...
    c.with = function(self, ...)
        ...
        for _, mixin in ipairs({ ... }) do
            assert(self.mixins[mixin] ~= true,
                ('Attempted to include a mixin which was already included in %s'):format(tostring(self)))
    ...
    c.without = function(self, ...)
        ...
        for _, mixin in ipairs({ ... }) do
            assert(self.mixins[mixin] == true,
                ('Attempted to remove a mixin which is not included in %s'):format(tostring(self)))
